...
namespace BundleConfigExtensions
{
    ...
    public static class BundleConfigExtensions
    {
        ...
        /// <exception cref="DuplicateBundleException">Throws DuplicateBundleException when the boolean parameter is passed as true and the bundle name is already exists in the MVC bundle.</exception>
        /// <exception cref="System.Exception">1. Exception thrown when the bundle virtual path was already added in the BundleCollection.
        /// 2. If the unsupported bundle type is mentioned in the bundle configuration file.
        /// 3. If the bundle configuration file has invalid content.</exception>
            ...
            bool throwErrorWhenDuplicateBundleFound)
        {
            ...
            foreach (var bundleDefinition in bundleDefinitions)
            {
                ...
                // Check if the bundle is already added to the MVC bundle collection
                if (throwErrorWhenDuplicateBundleFound && mvcBundleConfig.GetBundleFor(bundleDefinition.BundleVirtualPathOrName) != null)
                {
                    throw new DuplicateBundleException($"The bundle {bundleDefinition.BundleVirtualPathOrName} in {bundleConfigFileFullPath} file is already available in the bundle configuration. Cannot add duplicate bundle.");
                }
        ...
        /// <param name="throwErrorWhenDuplicateBundleFound">if set to <c>true</c> [throw error when duplicate bundle found].</param>
        /// <exception cref="System.FormatException">Throws System.FormatException when the format of the json file is invalid.</exception>
        /// <exception cref="System.IO.FileNotFoundException">Throws System.IO.FileNotFoundException if the file is not found.</exception>
        public static void LoadBundleConfigFrom(
            this BundleCollection mvcBundleConfig,
            ...
            bool throwErrorWhenDuplicateBundleFound)
        {
            ...
            var bundleConfigFiles = GetBundleConfigFilesFromDirectory(directoryPath, fileNameOrSearchPattern, includeSubdirectories);
        ...
        /// <returns>
        /// MVC bundle instance without including the bundle files
        /// </returns>
        /// <exception cref="System.Exception"></exception>
        private static Bundle GetMVCBundle(BundleDefinitionJson bundleDefinition, string bundleConfigFile)
        {
        ...
        {
            Bundle mvcBundle = GetMVCBundle(bundleDefinition, bundleConfigFile);
        ...
        /// <summary>
        /// Gets the bundle config files from directory.
        /// </summary>
        /// <param name="directoryPath">The directory path.</param>
        /// <param name="fileNameOrSearchPattern">The file name or search pattern.</param>
        /// <param name="includeSubdirectories">if set to <c>true</c> [include subdirectories].</param>
        ...
        private static string[] GetBundleConfigFilesFromDirectory(
            string directoryPath,
            string fileNameOrSearchPattern,
            bool includeSubdirectories)
        {
            ...
            string bundleconfigFullPath)
        {
            ...
            {
                throw new FormatException($"Unable to parse the bundle config file {bundleconfigFullPath}.", exp);
            }
        }
    }
}
